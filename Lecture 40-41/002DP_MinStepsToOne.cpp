...
int f(int n) {
	...
	// option 2 : reduce n to n/2 only when n%2 == 0
	...
	// option 3 : reduce n to n/3 only when n%3 == 0
...
// time : O(n)
// space : n (for fn call stack) + n (for dp[]) ~ O(n)
...
int fBottomUp(int n) {
	...
	for (int i = 2; i <= n; i++) {
		...
		// option 2 : reduce i to i/2 only when i%2 == 0
		...
		// option 3 : reduce i to i/3 only when i%3 == 0
