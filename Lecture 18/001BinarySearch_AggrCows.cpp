...
bool canPlaceCows(int c, int n, int pos[], int m) {
	...
	// place the remaining C-1 cows in the remaining N-1 stalls such that the min. dist. b/w cows is >= m

	for (int i = 1; i < n; i++) { // n-1.const ~ O(n)
	...
	// you cannot place c cows in n stalls at a dist of >= m
...
int largestMinDist(int n, int c, int pos[]) {
	...
	while (s <= e) { // log(e-s).n ~ O(nlog(e-s)) where e-s is the size of the search space
		int m = s + (e - s) / 2;
