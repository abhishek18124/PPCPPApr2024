	...
	bool dfsHelper(T source,
		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				// check if there is a directed path from neighbour to source
				if (stackState.find(neighbour) != stackState.end()) {
					// neighbour is present on the fn call stack
					// therefore there is a directed path from neighbour to source
