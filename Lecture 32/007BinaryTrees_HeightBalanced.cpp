...
bool checkHeightBalanced(TreeNode* root) {
	...
	if (root == NULL) {
		return true; // assume empty tree is heightbalance since no prop. is violated
	}
	...
	// 2. ask your friend to check if the rightSubtre is heightBalanced

	bool rightIsBal = checkHeightBalanced(root->right);

	// 3. check if the heightBalance property works at the root
...
Pair checkHeightBalancedEfficient(TreeNode* root) {
	...
	// 1. ask your friend to check if the leftSubtree is heightBalanced and also find its height

	Pair left = checkHeightBalancedEfficient(root->left);

	// 2. ask your friend to check if the rightSubtre is heightBalanced  and also find its height

	Pair right = checkHeightBalancedEfficient(root->right);

	// 3. check if the heightBalance property works at the root

	bool rootIsBal = abs(left.height - right.height) <= 1 ? true : false; // const

	p.isBalanced = left.isBalanced and right.isBalanced and rootIsBal; // const
	p.height = 1 + max(left.height, right.height); // const
...
pair<int, bool> checkHeightBalancedEfficient2(TreeNode* root) {
	...
	bool rootIsBal = abs(left.first - right.first) <= 1 ? true : false; // const

	p.second = left.second and right.second and rootIsBal; // const
	p.first = 1 + max(left.first, right.first); // const
...
int main() {
	...
	pp.second ? cout << "height-balanced!" << endl :
	                 cout << "not height-balanced" << endl;
