...
void f(char inp[], int i) {
	...
	// f(i) = replace all pi with 3.14 in inp[i...n-1]

	if (inp[i] == 'p' and inp[i + 1] == 'i') {

		// replace "pi" at the (i, i+1)th index with "3.14"

		// starting from index i+2, shift all the char. 2-steps to the right
		...
		// ask your friend to replace all pi with 3.14 in inp[i+4...n-1]

		f(inp, i + 4);

	} else {

		// ask your friend to replace all pi with 3.14 in inp[i+1....n-1]
...
int main() {
	...
	char inp[20] = "ppipipii"; // based on constraints
